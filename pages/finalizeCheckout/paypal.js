...
export default function paypal() {
  ...
  // sets default state of paypal email input
  const [payPalEmail, setPayPalEmail] = useState('')

  // handle paypal email changing values
  const handlePayPalEmail = event => {
    // set paypal email to user value
    setPayPalEmail(event.target.value)
  }

  // sets default state of paypal pass input
  const [payPalPass, setPayPalPass] = useState('')
  
  // handle paypal pass changing values
  const handlePayPalPass = event => {
    // set paypal pass to user value
  ...
  const handleCreditForm = event => {
    ...
    if (payPalEmail.length == 0) {
      // user did not input paypal email info
      // warn them to enter a value in the paypal email input
      toast.error('You must enter a value in the PayPal Email prompt', {
    ...
    } else if (payPalPass.length == 0) {
      // user did not input paypal pass info
      // warn them to enter a value in the paypal pass input
      toast.error('You must enter a value in the PayPal Pass prompt', {
        position: "bottom-right",
        autoClose: 4000,
        hideProgressBar: false,
        closeOnClick: true,
          ...
          <div className="flex flex-col sm:flex-row items-start justify-center w-full gap-4">
            ...
            <div className="w-full sm:w-1/2">
              ...
              <div className="space-y-3 mt-3">
                ...
                <input value={payPalPass} onChange={handlePayPalPass} htmlFor="PayPal Password" placeholder="PayPal Password" type="text" className="w-full rounded py-3 px-4 outline-none focus:bg-amber-100"/>
              </div>
